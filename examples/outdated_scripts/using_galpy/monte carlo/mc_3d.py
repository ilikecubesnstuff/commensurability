...
class OrbitalPhaseSpace:
    ...
    def load_into_array(self, shape, extent):
        ...
        xres, yres, zres = shape
        xmin, xmax, ymin, ymax = extent

        npzfile = np.load(self.file)
        if 'values' not in npzfile:
            values = np.zeros(shape)
            nearest = np.ones(shape) * np.inf
            # data = npzfile['raw']
        else:
            values = npzfile['values']
            nearest = npzfile['nearest']
    ...
    def generate_data_points(self, xrange, yrange, zrange, N=None, liveplot=False):
        if self.generated:
            npzfile = np.load(self.file)
            print(npzfile)
        ...
        data = []
        xres = yres = 500  # arbitrary!
        zres = 50  # arbitrary!
        shape = (xres, yres, zres)
        xmin, xmax = xrange
        ymin, ymax = yrange
        zmin, zmax = zrange
        dx = (xmax - xmin) / xres
        dy = (ymax - ymin) / yres
        dz = (zmax - zmin) / zres
        xinds, yinds, zinds = np.meshgrid(np.arange(xres), np.arange(yres), np.arange(zres))
        ...
        if liveplot:
            ...
            plt_values = ax_v.imshow(values[:,:,0], cmap='inferno', vmin=0, vmax=1, origin='lower', extent=extent, aspect=dx/dy)
            fig.colorbar(plt_values, ax=ax_v)
            plt_nearest = ax_n.imshow(nearest[:,:,0], cmap='inferno', vmin=0, vmax=min(1000, np.max(nearest)), origin='lower', extent=extent, aspect=dx/dy)
        ...
        try:
            for _ in it:
                ...
                dists = abs(xinds - xi) + abs(yinds - yi) + abs(zinds - zi)
                mask = (dists < nearest)
                if not np.sum(mask):
                    continue
                ...
                if liveplot:
                    plt_values.set_data(values[:,:,0])
                    plt_nearest.set_data(nearest[:,:,0])
                    fig.canvas.draw()
...
def default(liveplot=False):
    ...
    xrange = [0, 10]; yrange = [0, 600]; zrange = [0, 6]
    # xrange = [0, 3]; yrange = [200, 400]
    # xrange = [0, 2.5]; yrange = [400, 600]
    # xrange = [2, 6]; yrange = [0, 200]
    # xrange = [2, 4]; yrange = [100, 200]
    oph.generate_data_points(xrange=xrange, yrange=yrange, zrange=zrange, liveplot=liveplot)
